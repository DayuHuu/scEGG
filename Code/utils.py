...
def load_data(data_path, dataset_str, PCA_dim, is_NE=True, n_clusters=20, K=None):
    ...
    data = pd.read_csv(DATA_PATH, index_col=0, sep='\t',encoding='utf-8')
...
def load_emb(genemap_path):
    # 加载嵌入向量
    embedding_vectors = {}
    with open(genemap_path, encoding='utf-8') as f:
    ...
    with open(genemap_path, encoding='utf-8') as f:
        for line in f:
            ...
            embedding_vectors[node] = embedding
    # 将嵌入向量按顺序存储在一个矩阵中
    embedding_matrix = np.zeros((len(embedding_vectors),dim))
    print(embedding_matrix.shape)
    for i, node in enumerate(sorted(embedding_vectors.keys())):
        embedding_matrix[i] = embedding_vectors[node]
